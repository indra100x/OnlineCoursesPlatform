...
export default function CourseDetailsPage() {
    ...
    const loadCourse = useCallback(async () => {
        ...
        try {
            const response = await api.get<{ course: StudentCourse }>(
                        ...
                        <div className="mt-4 flex flex-wrap items-center gap-4 text-sm">
                            <div className="flex items-center gap-1.5">
                                ...
                                <Link
                                    ...
                                    className="text-sm font-semibold text-white underline decoration-white/25 underline-offset-4 transition hover:text-[#ffd84d]"
                                >
                                    {course.teacher.name}
                                </Link>
                            </div>
                        ...
                        <div className="space-y-3">
                            ...
                            course.chapters.length === 0 ? (
                                <EmptyState
                                    ...
                                    description="Your teacher has not published any chapter content yet."
                                />
                            ) : (
                                course.chapters.map((chapter) => (
                                    <article
                                        key={chapter.id}
                                        className="brand-surface brand-card-hover p-5"
                                    >
                                        <div className="flex items-start gap-3">
                                            ...
                                            <div className="min-w-0 flex-1">
                                                <p className="text-[10px] font-bold tracking-wider text-[#2563eb] uppercase">
                                ...
                                <div className="mt-4 space-y-2.5">
                                    ...
                                    course.ratings.length === 0 ? (
                                        <EmptyState
                                            ...
                                            description="Be the first student to leave feedback for this course."
                                        />
                                    ) : (
                                        course.ratings.map(
                                            (rating: CourseRating) => (
                                                <div
                                                    key={rating.id}
                                                    className="rounded-[1.15rem] border border-black/8 bg-[#fffdf7] p-3.5"
                                                >
